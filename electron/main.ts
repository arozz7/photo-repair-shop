...
import { ExifToolService } from './lib/exiftool/ExifToolService.js';
...
if (!gotTheLock) {
    ...
    app.whenReady().then(() => {
        ...
        ipcMain.handle('profiles:getGeneric', async () => {
            try {
                const profilesDir = app.isPackaged 
                    ? path.join(process.resourcesPath, 'assets/profiles') 
                    : path.join(__dirname, '../../assets/profiles');
                
                if (!fs.existsSync(profilesDir)) return [];
                const files = fs.readdirSync(profilesDir).filter(f => f.endsWith('.jpg') || f.endsWith('.hdr'));
                return files.map(f => path.join(profilesDir, f));
            } catch (e) {
                console.error("[IPC] Failed to load generic profiles:", e);
                return [];
            }
        });
